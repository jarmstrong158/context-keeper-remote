...
// lazy D1 migrations -> dispatch to the stateless MCP handler. Anything that
// does not match a valid token URL returns a bare 404: the URL is the
// credential, so we never confirm or deny why a request failed.
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (!expected || !safeEqual(token, expected)) return notFound();
    ...
    try {
      ...
      return Response.json(
        { jsonrpc: "2.0", id: null, error: { code: -32603, message: `migration failed: ${message}` } },
        { status: 500 },
      );
    }

    const ctx: ToolContext = { db: env.DB, env };
